...
**Реальний токен за цим числом: `layout/gap/row/{bp}`** (8/8/8/8/8, підтверджено 03.08.2026 прямим читанням boundVariables на "Light — Duel arena", file `rdQH0d2MRRKFkgzCmCGLGw`, node `6002:20991`, усі 5 брейкпоінтів) — **НЕ те саме**, що `layout/gap/secondary/{bp}` (яке росте до 12 на lg/xl, див. `pages/duel-arena.md` розділ 2 і `rules.md`). Не плутати два токени, коли перевіряєш boundVariables карткового ряду.

**Card/Duel може бути і горизонтально-скрольованою каруселлю, і responsive fill-гріднем — залежить від брейкпоінта в конкретній збірці**: на "Light — Duel arena" (вище) картка фіксована 298px і скролиться на xs/sm (не влазить у колонку), але на md/lg/xl розтягується (`flex-[1_0_0]`, кап `card/duel/maxWidth:428`) так, щоб рівно 3 картки заповнювали контент-колонку без скролу. Не вважай "не N-up грід" (нижче) універсальним правилом для Card/Duel на ВСІХ брейкпоінтах — перевіряй кожен окремо.
