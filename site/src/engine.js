...
function drawCtl(){const d=BY[CUR],p=S(CUR),c=$('#ctl');if(!c)return;const act=document.activeElement,keep=act&&act.dataset&&act.dataset.ctlText;
 c.innerHTML=d.props.filter(x=>x.t!='slot'&&x.ctl!==false&&(!x.when||x.when(p))).map(x=>{const id='c-'+x.n;
  if(x.t=='bool')return `<label class="c-row c-bool"><span class="c-n">${x.n}</span><button type="button" role="switch" aria-checked="${!!p[x.n]}" class="w-sw ${p[x.n]?'checked':''}" data-ctl-bool="${x.n}"><span class="w-sw-tr"><span class="w-sw-ind"></span></span></button></label>`;
