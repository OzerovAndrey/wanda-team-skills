...
reg({id:'tooltip',name:'Tooltip',group:'overlays',figma:'8:41429',alias:'hint popover title',
 ...
 stage:'tall',extra:()=>`<section class="blk"><h2 class="t4">All 12 positions</h2><p class="hint" style="margin:-4px 0 12px">Hover or tab to any anchor.</p><div class="stage" style="min-height:0"><div class="tt-grid">${[['Top — Left','Top — Center','Top — Right'],['Left — Top','','Right — Top'],['Left — Center','','Right — Center'],['Left — Bottom','','Right — Bottom'],['Bottom — Left','Bottom — Center','Bottom — Right']].flat().map(ps=>ps?`<span class="w-anc">${BTN({variant:'Secondary',size:'md',icon:'info',iconOnly:true,aria:ps})}<span class="w-pop w-tt ${posCls(ps)}" role="tooltip">${ps}<i class="arr"></i></span></span>`:'<span></span>').join('')}</div></div></section>`,
